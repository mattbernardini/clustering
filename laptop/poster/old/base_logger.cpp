...
const static int MAX_BUFFER_SIZE = 1128;
const static int MSG_BUFFER = 1000;
...
{
    sink * new_sink = (sink*)malloc(sizeof(struct _sink)); 
    new_sink->file = fopen(file_name, mode);
    new_sink->level_to_log_above = logLevel;
    if (!new_sink->file)
        return -1;
    this->sinks->push_front(new_sink);
	return 1;
}
int BaseLogger::addSink(
    int fd,
